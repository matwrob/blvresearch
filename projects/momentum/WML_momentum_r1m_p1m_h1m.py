...
from blvresearch.core.portfolio import (
    PortfolioStrategy, Portfolio, PositiveMomentumStrategy, StockReturns,
...
TOP_DECILE = lambda series, size: list(series[-size:].index)
...
class PositiveSentiment_1M_1M_1M(PortfolioStrategy):
...
class PositiveImpDays_1M_1M_1M(PositiveMomentum_1M_1M_1M):
    ...
    def _get_input_data(self):
        ...
        for year, v in news_by_year.items():
            res = pd.concat([v.stack(), alpha_by_year[year].stack()], axis=1)
            res = res.rename(columns={0: 'news', 1: 'alpha'})
            res = add_important_day_column(res)
            result[year] = res['important_day'].unstack()
        return pd.concat(list(result.values()))
...
POSITIVE_111 = pd.concat([pos_sent_perf, pos_mom_perf, pos_alpha_perf,
