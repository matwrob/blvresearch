...
rebalance = rel.resample('BM', how='sum').index
...
########################################################################
######################### FINAL STEP ###################################
# here I get rid of positive portfolio holdings whenever there's holding 
# for that position within the negative Dynamic Portfolio (analogously for
# negative periodic portfolio and positive dynamic portfolio)
# we want to get momentum but we suspect surprises will change momentum

for i, d in enumerate(pos_portfolio.index):
    pos_portfolio.ix[d][ndyn.holdings.ix[d] == True] = False

for i, d in enumerate(neg_portfolio.index):
    neg_portfolio.ix[d][pdyn.holdings.ix[d] == True] = False
